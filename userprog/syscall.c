...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  if(*((int *) f->esp) == SYS_OPEN)
  {
    ...
    ins->fileloc = temp;
    list_push_back(&thread_current()->files, &ins->elem);
  ...
  if(*((int *) f->esp) == SYS_FILESIZE)
  {
      ...
      {
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             f->eax = file_length(list_entry(index, struct file_desc, elem)->fileloc);
             sema_up(&filesys_sema);
             return;
           }
      }
  ...
  if(*((int *) f->esp) == SYS_TELL)
  {
	 ...
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             f->eax = file_tell(list_entry(index, struct file_desc, elem)->fileloc);
             sema_up(&filesys_sema);
             return;
           }
      }
  ...
  if(*((int *) f->esp) == SYS_SEEK)
  {
	 ...
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             file_seek(list_entry(index, struct file_desc, elem)->fileloc, *(unsigned *) arg2);
             sema_up(&filesys_sema);
             return;
           }
      }
  ...
  if(*((int *) f->esp) == SYS_WAIT)
  {
	  ...
	  index = list_next(index))
      {
	 if(list_entry(index, struct child_desc, child_elem)->child_pid == *(int *) arg1)
           {
             if(list_entry(index, struct child_desc, child_elem)->wait_num >0)
             {
             valid = 0;
             }
             else
             {
              valid = 1;
              list_entry(index, struct child_desc, child_elem)->wait_num++;
             }
             break;
           }
      }
	 ...
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             ...
             if(stack_value == NULL)
      		return; 
             f->eax = file_write(list_entry(index, struct file_desc, elem)->fileloc, *(char **) arg2, *(int *) arg3);
             sema_up(&filesys_sema); 
             return;
           }
      }
	 ...
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             ...
             f->eax = file_read(list_entry(index, struct file_desc, elem)->fileloc,*(char **) arg2, *(int *) arg3);
             sema_up(&filesys_sema);
             return;
           }
      }
  ...
  if(*((int *) f->esp) == SYS_CLOSE)
  {
	 ...
	 if(list_entry(index, struct file_desc, elem)->fd == *(int *) arg1)
           {
             f->eax = file_close(list_entry(index, struct file_desc, elem)->fileloc);
struct file_desc * aa =  list_entry(index, struct file_desc, elem);
             list_remove(index);
free(aa);
             sema_up(&filesys_sema);
             return;
